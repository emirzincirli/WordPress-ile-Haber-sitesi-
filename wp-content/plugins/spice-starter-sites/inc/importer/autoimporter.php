...
class Spice_Starter_Sites_Importer_Auto extends WP_Importer
{
	...
	function import_start( $file ) {
		...
		if ( ! is_file($file) ) {
			echo '<p><strong>' . __( 'Sorry, there has been an error.', 'spice-starter-sites' ) . '</strong><br />';
			echo __( 'The file does not exist, please try again.', 'spice-starter-sites' ) . '</p>';
			die();
		}
	...
	function handle_upload() {
		...
		if ( isset( $file['error'] ) ) {
			...
			echo '<p><strong>' . __( 'Sorry, there has been an error.', 'spice-starter-sites' ) . '</strong><br />';
			printf( __( 'The export file could not be found at <code>%s</code>. It is likely that this was caused by a permissions problem.', 'spice-starter-sites' ), esc_html( $file['file'] ) );
			echo '</p>';
			return false;
		}
		...
		if ( $this->version > $this->max_wxr_version ) {
			...
			printf( __( 'This WXR file (version %s) may not be supported by this version of the importer. Please consider updating.', 'spice-starter-sites' ), esc_html($import_data['version']) );
			echo '</strong></p></div>';
		}
	...
	function get_authors_from_import( $import_data ) {
		...
		// no author information, grab it from the posts
			...
			foreach ( $import_data['posts'] as $post ) {
				...
				if ( empty( $login ) ) {
					printf( __( 'Failed to import author %s. Their posts will be attributed to the current user.', 'spice-starter-sites' ), esc_html( $post['post_author'] ) );
					echo '<br />';
					continue;
				}
		...
		foreach ( (array) $this->authors as $i => $data ) {
			...
			if ( ! $create_users ) {
				...
				if ( ! is_wp_error( $user_id ) ) {
					...
					printf( __( 'Failed to create new user for %s. Their posts will be attributed to the current user.', 'spice-starter-sites' ), esc_html($this->authors[$old_login]['author_display_name']) );
	...
	function process_categories() {
		...
		foreach ( $this->categories as $cat ) {
			...
			if ( ! is_wp_error( $id ) ) {
				...
				printf( __( 'Failed to import category %s', 'spice-starter-sites' ), esc_html($cat['category_nicename']) );
				if ( defined('IMPORT_DEBUG') && IMPORT_DEBUG )
	...
	function process_tags() {
		...
		foreach ( $this->tags as $tag ) {
			...
			if ( ! is_wp_error( $id ) ) {
				...
				printf( __( 'Failed to import post tag %s', 'spice-starter-sites' ), esc_html($tag['tag_name']) );
				if ( defined('IMPORT_DEBUG') && IMPORT_DEBUG )
					echo ': ' . $id->get_error_message();
				echo '<br />';
				continue;
	...
	function process_terms() {
		...
		foreach ( $this->terms as $term ) {
			...
			if ( ! is_wp_error( $id ) ) {
				...
				printf( __( 'Failed to import %s %s', 'spice-starter-sites' ), esc_html($term['term_taxonomy']), esc_html($term['term_name']) );
				if ( defined('IMPORT_DEBUG') && IMPORT_DEBUG )
					echo ': ' . $id->get_error_message();
				echo '<br />';
				continue;
	...
	function process_posts() {
		...
		foreach ( $this->posts as $post ) {
			...
			if ( ! post_type_exists( $post['post_type'] ) ) {
				printf( __( 'Failed to import &#8220;%s&#8221;: Invalid post type %s', 'spice-starter-sites' ),
			...
			if ( $post_exists && get_post_type( $post_exists ) == $post['post_type'] ) {
				printf( __('%s &#8220;%s&#8221; already exists.', 'spice-starter-sites'), $post_type_object->labels->singular_name, esc_html($post['post_title']) );
				echo '<br />';
				...
				if ( is_wp_error( $post_id ) ) {
					printf( __( 'Failed to import %s &#8220;%s&#8221;', 'spice-starter-sites' ),
				...
				foreach ( $post['terms'] as $term ) {
					...
					if ( ! $term_id ) {
						...
						if ( ! is_wp_error( $t ) ) {
							...
							printf( __( 'Failed to import %s %s', 'spice-starter-sites' ), esc_html($taxonomy), esc_html($term['name']) );
							if ( defined('IMPORT_DEBUG') && IMPORT_DEBUG )
	...
	function process_menu_item( $item ) {
		...
		if ( ! $menu_slug ) {
			_e( 'Menu item skipped due to missing menu slug', 'spice-starter-sites' );
			echo '<br />';
			return;
		}
		...
		if ( ! $menu_id ) {
			printf( __( 'Menu item skipped due to invalid menu slug: %s', 'spice-starter-sites' ), esc_html( $menu_slug ) );
			echo '<br />';
			return;
		} else {
	...
	function fetch_remote_file( $url, $post ) {
		...
		if ( $headers['response'] != '200' ) {
			@unlink( $upload['file'] );
			return new WP_Error( 'import_file_error', sprintf( __('Remote server returned error response %1$d %2$s', 'spice-starter-sites'), esc_html($headers['response']), get_status_header_desc($headers['response']) ) );
		}
